...
from utils import CONFIG_PATH, RESULTS_DIR, random_email, generate_strong_password
        ...
        while task_counter < max_tasks or len(running_futures) > 0:
            ...
            while len(running_futures) < concurrent_flows and task_counter < max_tasks:
                ...
                if max_tasks > 1 and task_counter % (max_tasks // 2) == 0:
                    print(f"[Info] Da gui {task_counter}/{max_tasks} yeu cau.")
                elif max_tasks == 1:
                    print(f"[Info] Da gui {task_counter}/{max_tasks} yeu cau.")

            time.sleep(0.5)

    print(f"\n[Result] - Tong: {max_tasks}, Success: {succeeded_tasks}, Failed: {failed_tasks}")
...
if __name__ == "__main__":
    ...
    choose_browser = str(data.get("choose_browser", "playwright")).lower()
    if choose_browser == "patchright":
        try:
            ...
            if exc.name != "patchright":
                raise
            raise RuntimeError(
                "OutlookRegister/config.json dang chon choose_browser='patchright' "
                "nhung package patchright chua duoc cai. Hay cai dependency hoac doi "
                "choose_browser sang 'playwright'."
            ) from exc
        ...
        print("Trinh duyet khong duoc ho tro, tu dong fallback sang playwright")
        from controllers.playwright_controller import PlaywrightController
        selected_controller = PlaywrightController()
